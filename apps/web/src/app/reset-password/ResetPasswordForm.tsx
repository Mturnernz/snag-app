 ...
 *
 * The browser client picks them up on its own: auth-js's `detectSessionInUrl`
 * runs on construction, and `_isImplicitGrantCallback` fires on the presence of
 * `access_token` whatever the configured flow type is. So the recovery session
 * exists by the time PASSWORD_RECOVERY (or the initial session) lands, and
 * updateUser can set the new password against it.
 */
