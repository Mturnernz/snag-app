...
test('the walkthrough survives the tab going away and coming back', async ({ page }) => {
  ...
  await expect(page.getByPlaceholder('Add something…')).toBeHidden();
});
...
test('a typed snag is not lost when the tab comes back', async ({ page }) => {
  ...
  await page.getByPlaceholder('Add something…').fill('Gutters need clearing');

  await leaveAndComeBack(page);

  await expect(page.getByPlaceholder('Add something…')).toHaveValue('Gutters need clearing');
});
