...
only (a kind heading is not a place you can put something). It opens the walkthrough with the room
already chosen, and the room step is passed over after the photo, because pressing + on the Kitchen
has plainly answered "which room" (Back still reaches it). `start.room` therefore distinguishes `null` — Whole house, chosen deliberately — from
...
- **Seen is used.** A reading the sheet laid into the boxes is marked `used` at *Add it*; only one
  nobody saw becomes a card. The House tab says how many are waiting — **N labels to check**, a
  pill absent at nought, and *Label to check* on each card — and a reading landing while the app
  is still open says so once in a toast. Still no notifications.
...
`HouseScreen.test.tsx` the pill and its absence at nought.
